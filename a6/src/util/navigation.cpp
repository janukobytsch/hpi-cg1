...
namespace
{
    static const QVector3D DEFAULT_EYE    = QVector3D(0.f, 3.f, 6.f);
...
void Navigation::panningProcess(const QPoint & mouse)
{
    ...
    const QPoint clamped(
        glm::clamp(0, m_camera.viewport().width(), mouse.x())
    ,   glm::clamp(0, m_camera.viewport().height(), mouse.y()));
...
void Navigation::rotatingProcess(const QPoint & mouse)
{
    ...
    const float wDeltaX = glm::degrees(delta.x() / m_camera.viewport().width());
    // setup the degree of freedom for vertical rotation within a single action
    const float wDeltaY = glm::degrees(delta.y() / m_camera.viewport().height());
...
,   float & vAngle) const
{
    ...
    static const QVector3D up(0.0, 1.0, 0.0);
    const float va = glm::degrees(glm::acos(
